...
func can_purchase_item(
	item_id: String, quantity: int, cost_per_unit: float, game_data: Dictionary
) -> Dictionary:
	"""Validación completa de compra - REEMPLAZA 18+ DUPLICACIONES MASIVAS"""
	var result = {"can_purchase": false, "errors": [], "warnings": []}
	...
	if not can_afford_cost(current_money, total_cost):
		result.errors.append(
			"Dinero insuficiente: %.2f (necesitas %.2f)" % [current_money, total_cost]
		)
...
func can_produce_item(
	station_id: String, quantity: int, recipe: Dictionary, game_data: Dictionary
) -> Dictionary:
	"""Validación completa de producción - REEMPLAZA DUPLICACIONES CRÍTICAS"""
	var result = {"can_produce": false, "errors": [], "missing_resources": {}}
	...
	for ingredient_id in recipe.keys():
		...
		if not has_sufficient_resource(ingredient_id, needed, available):
			...
			result.missing_resources[ingredient_id] = {
				"needed": needed, "available": available, "missing": needed - available
			}
