...
func animate_button_press(button: Node, callback: Callable = Callable()):
	...
	# Escala hacia abajo
	tween.tween_property(button, "scale", Vector2(BUTTON_PRESS_SCALE, BUTTON_PRESS_SCALE),
...
func animate_button_success(button: Node, success_color: Color = Color.GREEN):
	...
	tween.parallel().tween_property(button, "scale", Vector2(BUTTON_SUCCESS_SCALE, BUTTON_SUCCESS_SCALE),
		BUTTON_ANIMATION_TIME * 2 * animation_speed_multiplier)
	tween.parallel().tween_property(button, "modulate", success_color,
		BUTTON_ANIMATION_TIME * animation_speed_multiplier)

	# Volver a normal
	tween.parallel().tween_property(button, "scale", original_scale,
		BUTTON_ANIMATION_TIME * 2 * animation_speed_multiplier)
	tween.parallel().tween_property(button, "modulate", original_modulate,
...
func animate_button_error(button: Node, error_color: Color = Color.RED):
	...
	# Cambio de color
	tween.parallel().tween_property(button, "modulate", error_color,
		BUTTON_ANIMATION_TIME * animation_speed_multiplier)

	# Shake effect
	for i in range(4):
		tween.tween_property(button, "position",
			original_position + Vector2(randf_range(-SHAKE_AMOUNT, SHAKE_AMOUNT),
			randf_range(-SHAKE_AMOUNT, SHAKE_AMOUNT)),
			0.05 * animation_speed_multiplier)

	# Volver a posición y color original
	tween.tween_property(button, "position", original_position,
		BUTTON_ANIMATION_TIME * animation_speed_multiplier)
	tween.parallel().tween_property(button, "modulate", original_modulate,
		BUTTON_ANIMATION_TIME * 2 * animation_speed_multiplier)
...
func create_floating_text(parent: Node, text: String, start_pos: Vector2,
						  color: Color = Color.WHITE, font_size: int = 16):
	...
	tween.parallel().tween_property(label, "position", end_pos, FADE_TIME * 2 * animation_speed_multiplier)
	tween.parallel().tween_property(label, "modulate", Color.TRANSPARENT, FADE_TIME * 2 * animation_speed_multiplier)
...
func create_currency_gain_effect(parent: Node, amount: float, currency_type: String, start_pos: Vector2):
...
func animate_progress_fill(progress_bar: ProgressBar, from_value: float, to_value: float, duration: float = 0.5):
...
func animate_panel_open(panel: Control, open_scale: Vector2 = Vector2.ONE):
	...
	tween.parallel().tween_property(panel, "scale", open_scale, FADE_TIME * animation_speed_multiplier)
	tween.parallel().tween_property(panel, "modulate", Color.WHITE, FADE_TIME * animation_speed_multiplier)
...
func animate_panel_close(panel: Control, callback: Callable = Callable()):
	...
	tween.parallel().tween_property(panel, "scale", Vector2(0.9, 0.9), FADE_TIME * animation_speed_multiplier)
	tween.parallel().tween_property(panel, "modulate", Color.TRANSPARENT, FADE_TIME * animation_speed_multiplier)
...
func animate_tab_switch(current_tab: Control, new_tab: Control, direction: String = "left"):
	...
	var slide_distance = current_tab.size.x if direction in ["left", "right"] else current_tab.size.y
	...
	tween.parallel().tween_property(current_tab, "position", current_original_pos + slide_vector,
		0.2 * animation_speed_multiplier)
	tween.parallel().tween_property(new_tab, "position", new_original_pos,
		0.2 * animation_speed_multiplier)
...
func animate_achievement_celebration(achievement_node: Control):
	...
	tween.tween_property(achievement_node, "scale", Vector2(1.2, 1.2), 0.15 * animation_speed_multiplier)
	tween.parallel().tween_property(achievement_node, "rotation", 0.1, 0.15 * animation_speed_multiplier)

	tween.tween_property(achievement_node, "scale", Vector2(0.95, 0.95), 0.1 * animation_speed_multiplier)
	tween.parallel().tween_property(achievement_node, "rotation", -0.05, 0.1 * animation_speed_multiplier)

	tween.tween_property(achievement_node, "scale", original_scale, 0.1 * animation_speed_multiplier)
	tween.parallel().tween_property(achievement_node, "rotation", original_rotation, 0.1 * animation_speed_multiplier)

	# Crear texto flotante de celebración
	var parent = achievement_node.get_parent()
	if parent:
		create_floating_text(parent, "🏆 ¡LOGRO DESBLOQUEADO!",
			achievement_node.global_position, Color.GOLD, 20)
