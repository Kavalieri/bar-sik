...
func test_production_economy_integration():
	...
	game_data.stations["lager_station"] = {
		"level": 3, "production_rate": 2.0, "active": true, "total_produced": 0.0
	}

	game_data.stations["ale_station"] = {
		"level": 2, "production_rate": 1.5, "active": true, "total_produced": 0.0
	}
	...
	var total_produced = (
		game_data.stations["lager_station"].total_produced
		+ game_data.stations["ale_station"].total_produced
	)
	assert_almost_eq(
		total_produced, total_expected_production, 1.0, "Producción total no coincide con esperada"
	)
...
func test_prestige_system_integration():
	...
	if can_prestige:
		...
		assert_eq(
			game_data.prestige_level,
			initial_prestige_level + 1,
			"Nivel de prestigio debería incrementar"
		)
		assert_gt(
			game_data.prestige_tokens, initial_tokens, "Tokens de prestigio deberían incrementar"
		)
		assert_eq(game_data.money, 0.0, "Dinero debería resetearse a 0 tras prestigio")
...
func test_statistics_tracking_integration():
	...
	assert_eq(
		game_data.customers_served,
		initial_customers_served + 15,
		"Clientes atendidos no se rastreó correctamente"
	)
	assert_eq(
		game_data.beers_brewed,
		initial_beers_brewed + 25,
		"Cervezas producidas no se rastreó correctamente"
	)
	assert_almost_eq(
		game_data.total_earnings,
		initial_total_earnings + 1500.0,
		0.01,
...
func test_cross_manager_communication():
	...
	assert_gt(
		updated_production_stats.beers_produced,
	...
	assert_gt(
		new_production_multiplier,
		initial_production_multiplier,
...
func test_null_manager_handling():
	...
	assert_true(result is Dictionary or result is bool, "Resultado debería ser tipo esperado")
...
func test_invalid_data_recovery():
	...
	game_data.stations["corrupted_station"] = {
		"level": -5, "production_rate": "not_a_number", "active": null  # Nivel inválido  # Tipo incorrecto  # Valor nulo
	}
	...
	assert_true(
		processing_result.success or processing_result.error_handled,
...
func _initialize_game_systems():
	...
	game_data.stations["lager_station"] = {
		"level": 1, "production_rate": 1.0, "active": true, "total_produced": 0.0
	}
...
func _attempt_purchase_upgrade(upgrade_id: String, cost: float) -> bool:
	...
	if game_data.money >= cost:
		...
		game_data.upgrades[upgrade_id] = {"purchased": true, "cost": cost, "effect_applied": true}
		return true
	return false
