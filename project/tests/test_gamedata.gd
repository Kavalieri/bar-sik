...
func test_load_game_data():
	...
	game_data.stations["test_station"] = {
		"level": 3,
		"production_rate": 2.5,
		"active": false
	}
...
func test_save_data_integrity():
	...
	for i in range(3):
		var station_id = "station_" + str(i)
		assert_true(loaded_data.stations.has(station_id),
			"Estación " + station_id + " no encontrada")

		var station = loaded_data.stations[station_id]
		assert_eq(station.level, i + 1, "Nivel de estación " + station_id + " incorrecto")
		assert_eq(station.active, i % 2 == 0,
			"Estado activo de " + station_id + " incorrecto")
...
func test_negative_values_protection():
	...
	assert_true(money_valid or game_data.money == 0,
		"Dinero negativo debería ser manejado")
	assert_true(tokens_valid or game_data.prestige_tokens == 0,
		"Tokens negativos deberían ser manejados")
	assert_true(gems_valid or game_data.gems == 0,
		"Gemas negativas deberían ser manejadas")
...
func test_large_numbers_handling():
	...
	assert_almost_eq(game_data.total_earnings, large_earnings, 1e9,
		"Números muy grandes no manejados")
	...
	assert_almost_eq(loaded_data.money, large_money, 1e6,
		"Precisión de números grandes perdida en save/load")
