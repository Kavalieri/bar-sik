...
func test_progress_bar_functionality():
	...
	assert_lte(
		progress_bar.value, progress_bar.max_value, "Barra de progreso no respeta valor máximo"
	)

	# Verificar porcentaje
	var percentage = (progress_bar.value / progress_bar.max_value) * 100
	assert_gte(percentage, 0.0, "Porcentaje no puede ser negativo")
	assert_lte(percentage, 100.0, "Porcentaje no puede exceder 100%")
...
func test_money_display_binding():
	...
	assert_true(money_label.text.contains("2500"), "Display de dinero no refleja valor correcto")
	assert_true(
		money_label.text.contains("$") or money_label.text.contains("Money"),
...
func test_production_info_binding():
	...
	assert_true(
		(
			production_label.text.contains(str(production_rate))
			or production_label.text.contains("5.5")
		),
		"Display de producción no muestra tasa correcta"
	)
	assert_true(
		production_label.text.contains(str(active_stations)) or production_label.text.contains("3"),
