...
func _update_single_station_state(station_id: String, game_data: Dictionary = {}):
	...
	if is_unlocked:
		...
		state.buy_button.text = "PRODUCIR x%d" % multiplier if can_produce else "SIN INGREDIENTES"
	else:
		# Modo desbloqueo - verificar dinero disponible
		...
		state.buy_button.text = "DESBLOQUEAR" if can_afford else "SIN DINERO"
